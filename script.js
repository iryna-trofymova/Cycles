"use strict";
const numbers = [5, 8, -1, "-6", 11, "ololo", "-2"];
let sum = 0;
for (let i = 0; i < numbers.length; i++) {
  sum += numbers[i];
}

alert("Sum is " + sum);

/*1)
1. Оголоcити numbers
    2. присвоїти numbers масив з 7 елементів 5, 8, -1, "-6", 11, "ololo", "-2" :numbers = [5, 8, -1, "-6", 11, "ololo", "-2"]
    3. оголосити sum
    4. присвоїти sum значення 0; sum = 0;
    5. оголосити i
    6. присвоїти i значення 0: i = 0
        7. перевірити, чи i менше довжини масиву: 0 < 7 - так
    8. присвоїти  sum попереднє значення, до якого додали 0-й елемент масиву:  sum  = 0 + 5 = 5
    9. збільшити i на одиницю: i = 1
    10. перевірити, чи i менше довжини масиву: 1 < 7 - так
    11. присвоїти  sum  попереднє значення, до якого додали 1-й елемент масиву:  sum  = 5 + 8 = 13
    12. збільшити i на одиницю: i = 2
    13. перевірити, чи i менше довжини масиву: 2 < 7 - так
    14. присвоїти  sum  попереднє значення, до якого додали 2-й елемент масиву:  sum  = 13 + (-1) = 12
    15. збільшити i на одиницю: i = 3
    16. перевірити, чи i менше довжини масиву: 3 < 7 - так
        17. присвоїти  sum  попереднє значення, до якого додали 3-й елемент масиву:  sum  = 12 +"-6" = 5-6
    18. збільшити i на одиницю: i = 4
        19. перевірити, чи i менше довжини масиву: 4 < 7 - так
    20. присвоїти  sum  попереднє значення, до якого додали 4 - й елемент масиву: sum = "5-6" + 11 = 5 - 611
    21. збільшити i на одиницю: i = 5
        22. перевірити, чи i менше довжини масиву: 5 < 7 - так
    23. присвоїти  sum  попереднє значення, до якого додали 5 - й елемент масиву: sum = "5 - 611" + "ololo"= 5 - 611ololo
    24. збільшити i на одиницю: i = 6
        25. перевірити, чи i менше довжини масиву: 6 < 7 - так
    26. присвоїти  sum  попереднє значення, до якого додали 6 - й елемент масиву: sum = " 5 - 611ololo" + "-2" = 5 - 611ololo-2
    27. збільшити i на одиницю: i = 7
    28. перевірити, чи i менше довжини масиву: 7 < 7 - ні
        29. вивести повідомлення Sum is 5 - 611ololo-2


2)  програма перестала коректно рахувати суму чисел на п'ятій ітерації. і=4.

3) пофіксити баг, не змінюючи сам масив. тобто виправити баг таким чином, щоб сума рахувалася правильно і видавало 15*/

const newNumbers = [5, 8, -1, "-6", 11, "ololo", "-2"];
let summ = 0;
for (let i = 0; i < newNumbers.length; i++) {
  const Number = +newNumbers[i];
  if (!isNaN(Number)) {
    summ += Number;
  }
}
alert("Sum is " + summ);
